/*
=====================================================
2026 Fix: confirm_stock_transfer_received ambiguous id

Purpose:
Qualifies the `returning id` reference in the
READY_FOR_PICKUP branch of confirm_stock_transfer_received.
The function is declared RETURNS TABLE(id uuid, ...), which
makes `id` a PL/pgSQL output-parameter variable, so the
unqualified `returning id` in the inventory INSERT was
ambiguous (variable vs inventory.id column) and raised
ERROR 42702, surfacing as HTTP 400.

Note:
This migration is the first of two incremental fixes that
were applied to the live database. It resolves the
`returning id` ambiguity only; the `where id = ...` in the
fulfillment UPDATE is still ambiguous here and is fixed by
