...
create or replace function public.submit_bhw_medicine_request_idempotent(
  p_operation_id uuid,
  p_items jsonb,
...
begin
  ...
  update private.offline_operation_receipts
     set result_id = v_request_id
   where user_id = auth.uid() and operation_id = p_operation_id;
  return v_request_id;
end;
$$;

create or replace function public.create_manual_medicine_request_idempotent(
  p_operation_id uuid,
  p_facility_id uuid,
  p_items jsonb,
...
begin
  ...
  v_request_id := public.create_manual_medicine_request(
    p_facility_id, p_items, p_manual_requested_by, p_remarks
  );
  update private.offline_operation_receipts
     set result_id = v_request_id
   where user_id = auth.uid() and operation_id = p_operation_id;
...
create or replace function public.submit_bhw_stock_transfer_request_idempotent(
  p_operation_id uuid,
...
begin
  ...
  update private.offline_operation_receipts
     set result_id = v_transfer_id
   where user_id = auth.uid() and operation_id = p_operation_id;
  return v_transfer_id;
end;
$$;

create or replace function public.create_cho_stock_transfer_idempotent(
  p_operation_id uuid,
  p_source_facility_id uuid,
...
begin
  ...
  v_transfer_id := public.create_cho_stock_transfer(
    p_source_facility_id, p_destination_facility_id, p_items, p_remarks
  );
  update private.offline_operation_receipts
     set result_id = v_transfer_id
   where user_id = auth.uid() and operation_id = p_operation_id;
