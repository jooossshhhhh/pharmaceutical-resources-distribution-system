...
begin
  select p.facility_id, p.role in ('PHARMA_I', 'PHARMA_II')
    into v_caller_facility, v_is_cho
    from public.profiles p
   where p.id = auth.uid()
     and p.status = 'ACTIVE';
  ...
  if v_caller_facility <> v_source_facility and not v_is_cho then
...
declare
  ...
  v_transfer public.stock_transfers%rowtype;
begin
  select p.id, p.facility_id, p.role in ('PHARMA_I', 'PHARMA_II')
    into v_caller_id, v_caller_facility, v_is_cho
    from public.profiles p
   where p.id = auth.uid()
     and p.status = 'ACTIVE';
  ...
  if v_caller_facility <> v_transfer.source_facility_id and not v_is_cho then
    raise exception 'Only the source facility can allocate this transfer.'
      using errcode = '42501';
  end if;
