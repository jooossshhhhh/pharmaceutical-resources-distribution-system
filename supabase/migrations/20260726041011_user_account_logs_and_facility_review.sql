...
create or replace function public.log_own_password_change()
returns void
language plpgsql
security definer
set search_path = public
...
revoke all on function public.log_own_password_change() from public;
grant execute on function public.log_own_password_change() to authenticated;

create or replace function public.notify_and_log_facility_change_request()
returns trigger
language plpgsql
security definer
set search_path = public
...
for each row execute function public.notify_and_log_facility_change_request();

create or replace function public.review_profile_facility_change_request(
    p_request_id uuid,
...
revoke all on function public.review_profile_facility_change_request(uuid, facility_change_request_status) from public;
grant execute on function public.review_profile_facility_change_request(uuid, facility_change_request_status) to authenticated;;
