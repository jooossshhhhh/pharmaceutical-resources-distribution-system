...
Adds reservation-aware CHO availability and atomic BHW
...
begin
  ...
  with physical_stock as (
    select i.medicine_id, sum(i.quantity)::bigint as quantity
  ...
  reserved_stock as (
    select ri.medicine_id, sum(ri.quantity)::bigint as quantity
  ...
  select
    ...
    m.unit_of_measure,
    ps.quantity,
    coalesce(rs.quantity, 0)::bigint,
    greatest(ps.quantity - coalesce(rs.quantity, 0), 0)::bigint
