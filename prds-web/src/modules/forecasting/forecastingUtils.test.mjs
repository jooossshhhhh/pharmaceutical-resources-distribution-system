...
test("builds inventory coverage from current stock and latest forecast demand", () => {
  ...
  assert.equal(coverageRows[0].risk.label, "Low");
});
...
test("labels inventory risk using stock coverage and threshold", () => {
  ...
  assert.equal(getInventoryRisk({ coverageMonths: 0.5, quantity: 30, threshold: 20 }).label, "Low");
  assert.equal(getInventoryRisk({ coverageMonths: 1.25, quantity: 40, threshold: 20 }).label, "Watch");
  assert.equal(getInventoryRisk({ coverageMonths: 2.5, quantity: 80, threshold: 20 }).label, "Stable");
});
