...
test("matches keyword, category, status, role, and facility filters", () => {
  ...
  assert.equal(
    matchesNotificationFilters(notifications[0]),
    true
  );
...
test("normalizes notification read updates from object and legacy arguments", () => {
  assert.deepEqual(
    normalizeNotificationReadArgs([
      { notificationId: "n-1", isRead: true, profileId: "user-1" },
    ]),
    { notificationId: "n-1", isRead: true, profileId: "user-1" }
  );
  assert.deepEqual(
    normalizeNotificationReadArgs(["n-2", false, "user-2"]),
    { notificationId: "n-2", isRead: false, profileId: "user-2" }
  );
});
