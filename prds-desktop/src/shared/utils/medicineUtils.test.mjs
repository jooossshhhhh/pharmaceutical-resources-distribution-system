...
test("medicine categories parse, deduplicate, and format", () => {
  assert.deepEqual(parseMedicineCategories("Anti-Infectives, HIV Program, Anti-Infectives"), [
    "Anti-Infectives",
    "HIV Program",
  ]);
  assert.equal(
    formatMedicineCategories(["Anti-Infectives", "HIV Program"]),
    "Anti-Infectives, HIV Program"
  );
});
...
test("findDuplicateMedicines separates exact duplicates from same generic matches", () => {
  ...
  const sameMedicineWithoutBrand = findDuplicateMedicines({
    medicines,
    formValues: {
      generic_name: "amoxicillin",
      brand_name: "",
  ...
  assert.deepEqual(
    sameMedicineWithoutBrand.exactMatches.map((medicine) => medicine.id),
    ["med-1"]
  );
