...
export const normalizeNotificationReadArgs = (args = []) => {
  const [firstArg, positionalReadState, positionalProfileId] = args;

  if (firstArg && typeof firstArg === "object") {
    return {
      notificationId: firstArg.notificationId,
      isRead: firstArg.isRead,
      profileId: firstArg.profileId,
    };
  }

  return {
    notificationId: firstArg,
    isRead: positionalReadState,
...
export const matchesNotificationFilters = (
  ...
  if (roleFilter && roleFilter !== "ALL" && notification.recipient_role !== roleFilter) {
    return false;
  }

  if (facilityId && facilityId !== "ALL" && notification.recipient_facility_id !== facilityId) {
    return false;
  }
