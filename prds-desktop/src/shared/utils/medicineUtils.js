...
export const parseMedicineCategories = (value) => {
  const values = Array.isArray(value) ? value : String(value || "").split(",");

  return [...new Set(values.map((category) => category.trim()).filter(Boolean))];
};

export const formatMedicineCategories = (categories) => {
  return parseMedicineCategories(categories).join(", ");
};
...
export const normalizeMedicineText = (medicine) => {
  return [
    ...
    medicine.unit_cost,
    ...parseMedicineCategories(medicine.categories),
  ]
    .filter(Boolean)
    .join(" ")
    .toLowerCase();
