...
test("facilities without inventory are marked as unassessed, not warned", () => {
  const view = buildFacilityView(facility, {
    inventory: [],
    requests: [],
    patients: [],
  ...
  assert.equal(view.healthPercent, null);
  assert.equal(view.stockHealth, "NO_DATA");
  assert.equal(getHealthMeta(view.stockHealth).label, "No stock data");
});

test("missing facility status has a readable fallback", () => {
