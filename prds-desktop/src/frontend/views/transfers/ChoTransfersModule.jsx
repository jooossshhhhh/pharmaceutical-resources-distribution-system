...
const updateCreateForm = (key, value) => {
  ...
  const handleCreateTransfer = async (event) => {
    ...
    setIsSaving(true);
    try {
      const result = await createChoStockTransfer({
      ...
      if (result?.queued) setNotice("Transfer queued offline. It will be submitted when connection returns; the server will validate stock before accepting it.");
      await loadData();
    } catch (createError) {
