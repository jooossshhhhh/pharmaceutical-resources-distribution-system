...
} from "@backend/services/transfersService";
import {
...
} from "@shared/utils/transferUtils";
import {
  getExpiryStatus,
  getMedicineName,
  getStockStatus,
...
function IncomingTransferModal({
  ...
  const {
    ...
    paginatedRows: paginatedFefoRows,
    pageSize: fefoPageSize,
    setCurrentPage: setFefoPage,
    totalCount: fefoTotalCount,
    totalPages: fefoTotalPages,
  } = usePaginatedRows(fefoRows, 10);
...
export default function BhwTransfersModule() {
  ...
  const handleSubmitTransfer = async (event) => {
    ...
    setIsSaving(true);
    try {
      const result = await submitBhwStockTransferRequest({
      ...
      if (result?.queued) setNotice("Transfer request queued offline. It will be submitted when connection returns; stock is not reserved yet.");
      await loadData();
    } catch (submitError) {
