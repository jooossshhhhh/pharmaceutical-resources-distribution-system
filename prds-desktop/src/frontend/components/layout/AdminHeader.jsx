...
export default function AdminHeader({ profile, currentDateTime, onSignOut }) {
  ...
  useEffect(() => {
    ...
    const loadNotifications = async () => {
      ...
      const { notifications: data = [] } = await getNotificationData();

      if (!isMounted) {
        return;
      }

      setNotifications(data.filter((notification) => notification.user_id === profile.id).slice(0, 5));
      setHeaderError("");
    };
  ...
  const markNotificationsRead = async () => {
    ...
    try {
      await Promise.all(
        notifications
          .filter((notification) => notification.user_id === profile.id && !notification.is_read)
          .map((notification) =>
            updateOwnNotificationReadStatus({
